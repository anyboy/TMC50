...
HOSTCC       ?= gcc
HOSTCXX      ?= g++
HOSTCFLAGS   = -Wall -Werror -Wmissing-prototypes -Wstrict-prototypes -O2 -fomit-frame-pointer -std=gnu89
HOSTCXXFLAGS = -O2
...
sdk_version_h := include/generated/sdk_version.h
...
no-dot-config-targets := pristine distclean clean mrproper help kconfig-help host-tools \
			 ...
			 $(version_h) $(sdk_version_h) headers_% kernelversion %src-pkg
...
# output colorful diagnostics message
KBUILD_CFLAGS += $(call cc-option,-fdiagnostics-color,)

-include $(srctree)/ext/Makefile
-include $(srctree)/lib/Makefile
-include $(srctree)/drivers/wifi/Makefile_lib
...
ifneq (${ZEPHYR_GCC_VARIANT},mipssde)
...
KALLSYMS_OBJ = $(objtree)/.tmp_kallsyms1.o
else
KALLSYMS_OBJ =
endif

ifeq ($(CONFIG_KALLSYMS_NO_NAME),y)
KALLSYMS_PARAM = --no_symbols_name
else
KALLSYMS_PARAM =
endif
...
$(PREBUILT_KERNEL): $(zephyr-deps) libzephyr.a libapplication.a linker.cmd $(KERNEL_NAME).lnk
#	$(Q)$(CC) -T linker.cmd @$(KERNEL_NAME).lnk -o .tmp_$@ && \
#	${NM} -n .tmp_$@ | scripts/kallsyms | ${CC} $(KBUILD_AFLAGS) -c -o $(KALLSYMS_OBJ) -x assembler-with-cpp - && \
#	$(CC) -T linker.cmd @$(KERNEL_NAME).lnk -o $@ $(KALLSYMS_OBJ)

	$(Q)$(CC) -T linker.cmd @$(KERNEL_NAME).lnk -o .tmp_$@ &&	\
	if [ -n "$(CONFIG_KALLSYMS)" ]; then \
		${NM} -n .tmp_$@ | scripts/kallsyms $(KALLSYMS_PARAM) | ${CC} $(KBUILD_AFLAGS) -c -o $(KALLSYMS_OBJ) -x assembler-with-cpp - && \
		$(CC) -T linker.cmd @$(KERNEL_NAME).lnk -o $@ $(KALLSYMS_OBJ); \
	else \
		mv .tmp_$@ $@;	\
	fi;
...
$(KERNEL_ELF_NAME): $(GENERATED_KERNEL_OBJECT_FILES) linker-pass2.cmd
	$(Q)$(CC) -T linker-pass2.cmd $(GENERATED_KERNEL_OBJECT_FILES) \
		@$(KERNEL_NAME).lnk -o $@ $(KALLSYMS_OBJ)
	$(Q)$(srctree)/scripts/check_link_map.py $(KERNEL_NAME).map
	$(Q)$(srctree)/scripts/support/actions/map_parse.py $(KERNEL_NAME).map
	@$(WARN_ABOUT_ASSERT)
...
$(KERNEL_BIN_NAME): $(KERNEL_ELF_NAME)
	$(call cmd,gen_bin)
ifneq ($(SYNC_BUILD),1)
	$(OBJDUMP) -S $< > $(KERNEL_NAME).lst && echo "LST     $(KERNEL_NAME).lst finished!" &
else
	$(OBJDUMP) -S $< > $(KERNEL_NAME).lst && echo "LST     $(KERNEL_NAME).lst finished!"
endif
...
prepare1: prepare2 $(version_h) $(sdk_version_h)\
                   include/config/auto.conf
...
KERNEL_BUILD_DATE=$(shell date +%y%m%d)
KERNEL_BUILD_TIME=$(shell date +%H%M%S)
...
define filechk_version.h
       ...
       echo "#define KERNEL_BUILD_DATE         \"$(KERNEL_BUILD_DATE)\""; \
       echo "#define KERNEL_BUILD_TIME         \"$(KERNEL_BUILD_TIME)\""; \
       echo; \
       echo "#endif /* _KERNEL_VERSION_H_ */";)
endef

USER_NAME=$(shell printf '%s\n' $(USER))
SDK_NAME=ZS283A
BT_DEVICE_NAME=$(SDK_NAME)-$(USER_NAME)
BLE_DEVICE_NAME=$(SDK_NAME)-BLE-$(USER_NAME)

define filechk_sdk_version.h
       (echo "#ifndef _SDK_VERSION_H_"; \
       echo "#define _SDK_VERSION_H_"; \
       echo ;\
       echo "#define SDK_BT_DEVICE_NAME         \"$(BT_DEVICE_NAME)\""; \
       echo "#define SDK_BLE_DEVICE_NAME        \"$(BLE_DEVICE_NAME)\""; \
       echo; \
       echo "#endif /* _SDK_VERSION_H_ */";)
endef

$(version_h): $(srctree)/Makefile FORCE
	$(call filechk,version.h)

$(sdk_version_h): $(srctree)/Makefile FORCE
	$(call filechk,sdk_version.h)	
...
MRPROPER_FILES += .config .config.old .version $(version_h) $(sdk_version_h)\
...
nvram_bin:
	$(Q)mkdir -p $(FW_DIR)/bin &&	\
	$(PYTHON) -B $(srctree)/scripts/support/actions/build_nvram_bin.py \
		-o $(FW_DIR)/bin/nvram.bin \
		$(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/nvram.prop \
		$(PROJECT_BASE)/nvram.prop \
		$(PROJECT_BASE)/app_conf/${APP_CONFIG}/nvram.prop \

datafs_bin:
	$(Q)if [ "$(CONFIG_DISK_ACCESS_NOR)" = "y" ] && [ -d $(PROJECT_BASE)/app_conf/${APP_CONFIG}/datafs ]; then	\
		$(PYTHON) -B $(srctree)/scripts/support/actions/build_datafs.py \
			-o $(FW_DIR)/bin/rodatafs.bin \
			-s $(CONFIG_DISK_NOR_VOLUME_SIZE) \
			-d $(PROJECT_BASE)/app_conf/${APP_CONFIG}/datafs; \
	else	\
		dd if=/dev/zero of=$(FW_DIR)/bin/rodatafs.bin bs=512 count=1; \
	fi;

sdfs_bin:
	$(Q)if [ -d $(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/sdfs ]; then \
			$(Q)mkdir -p $(O)/sdfs; \
			echo 'SDFS: copy sdfs for $(BOARD_NAME) ...'; \
			cp -rf  $(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/sdfs/* $(O)/sdfs/; \
	fi;

	$(Q)if [ -d $(PROJECT_BASE)/sdfs ]; then \
			$(Q)mkdir -p $(O)/sdfs; \
			echo 'SDFS: copy sdfs for common ...'; \
			cp -rf  $(PROJECT_BASE)/sdfs/* $(O)/sdfs/; \
	fi;

	$(Q)if [ -d $(O)/sdfs ]; then \
		$(PYTHON) -B $(srctree)/scripts/support/actions/build_sdfs.py \
			-o $(FW_DIR)/bin/sdfs.bin \
			-d $(O)/sdfs; \
	fi;

firmware: nvram_bin datafs_bin sdfs_bin
	@echo "FW: Generate firmware for $(BOARD_NAME) ..."
	$(Q)cp -r  $(srctree)/scripts/support/$(SOC_FAMILY)/prebuilt/$(SOC_SERIES)/common/* $(FW_DIR)/
	$(Q)cp -r  $(srctree)/scripts/support/$(SOC_FAMILY)/prebuilt/$(SOC_SERIES)/$(CONFIG_SOC)/* $(FW_DIR)/

	$(Q)if [ "$(CONFIG_OTA_RECOVERY)" = "y" ]; then	\
		echo 'FW: Copy recovery.bin';		\
		cp $(O)/../$(BOARD_NAME)_recovery/$(KERNEL_BIN_NAME) $(FW_DIR)/bin/recovery.bin; \
	fi;

	$(Q)if [ -f $(O)/../$(BOARD_NAME)_second/$(KERNEL_BIN_NAME) ]; then	\
		echo 'FW: Copy zephyr_second.bin';		\
		cp $(O)/../$(BOARD_NAME)_second/$(KERNEL_BIN_NAME) $(FW_DIR)/bin/zephyr2.bin; \
	fi

	$(Q)cp $(O)/$(KERNEL_BIN_NAME) $(FW_DIR)/bin/

	$(Q)if [ -f $(PROJECT_BASE)/firmware.xml ] ; then	\
		echo 'FW: Override the default firmware.xml';	\
		cp $(PROJECT_BASE)/firmware.xml $(FW_DIR)/;	\
	fi;

	$(Q)if [ -f $(O)/../$(BOARD_NAME)/firmware.xml ] ; then	\
		echo 'FW: Override the default firmware.xml';	\
		cp $(O)/../$(BOARD_NAME)/firmware.xml $(FW_DIR)/;	\
	fi;

	$(Q)if [ -f $(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/${ECHECK_FW} ] ; then		\
		cp $(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/${ECHECK_FW} $(FW_DIR)/bin/;	\
	else	\
		echo '$(ZEPHYR_BASE)/boards/$(ARCH)/$(BOARD_NAME)/${ECHECK_FW} NOT exist'; \
		exit 1;	\
	fi;

	$(Q)if [ "$(CONFIG_ACTIONS_ATT)" = "y" ] && [ -d $(PROJECT_BASE)/att ]; then	\
		if [ -d $(PROJECT_BASE)/att ]; then	\
			rm -rf $(FW_DIR)/att;		\
		fi;					\
		cp -r $(PROJECT_BASE)/att $(FW_DIR)/;						\
	fi;

	$(Q)$(PYTHON) -B $(srctree)/scripts/support/actions/build_firmware.py \
		-c $(FW_DIR)/firmware.xml \
		-e $(FW_DIR)/bin/encrypt.bin \
		-ef $(FW_DIR)/bin/${ECHECK_FW} \
		-s $(CONFIG_SOC_SERIES) \
		-b $(BOARD_NAME)

update-sdk-library: $(SDK_LIB)
	@echo "sdk library is updated successfully"

$(SDK_LIB):
	@echo "gererate SDK library $(SDK_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(SDK_LIB_OUT_DIR)
	$(Q)rm -f $(SDK_LIB_OUT_DIR)/$@.a
	$(Q)find $(SDK_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(SDK_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(SDK_LIB_OBJ_DIR)/$@.a $(SDK_LIB_OUT_DIR)/$@.a

update-soc-library: $(SOC_LIB)
	@echo "soc library is updated successfully $(SOC_LIB)"

$(SOC_LIB):
	@echo "gererate SOC library $(SOC_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(SOC_LIB_OUT_DIR)
	$(Q)rm -f $(SOC_LIB_OUT_DIR)/$@.a
	$(Q)find $(SOC_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(SOC_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(SOC_LIB_OBJ_DIR)/$@.a $(SOC_LIB_OUT_DIR)/$@.a


update-private-library: $(PRIVATE_LIB)
	@echo "private library is updated successfully $(PRIVATE_LIB)"

$(PRIVATE_LIB):
	@echo "gererate private library $(PRIVATE_LIB_OUT_DIR)/$@.a"
	$(Q)mkdir -p $(PRIVATE_LIB_OUT_DIR)
	$(Q)rm -f $(PRIVATE_LIB_OUT_DIR)/$@.a
	$(Q)find $(PRIVATE_LIB_OBJ_DIR)/$@ -type f -name "*.o"  ! -name "built-in.o" -print | xargs $(AR) rcs $(PRIVATE_LIB_OBJ_DIR)/$@.a
	$(Q)cp $(PRIVATE_LIB_OBJ_DIR)/$@.a $(PRIVATE_LIB_OUT_DIR)/$@.a
