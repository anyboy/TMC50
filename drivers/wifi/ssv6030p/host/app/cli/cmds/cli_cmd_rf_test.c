...
#define OP_NUM 48
const static char *RADIO_OP[OP_NUM] =
{
"ch", //"CHANNEL=",
"WIFI_OPMODE=",
"11N_CONFIG=",
...
void RFStart()
{
    ...
    if(TRUE != ssv_hal_rf_load_default_setting())
        ASSERT(0);
    ...
    ssv6xxx_set_voltage_mode(g_host_cfg.volt_mode);
    ...
    ssv6xxx_drv_write_reg(0xCE01002C,0x00030CA8); //SET_RG_TX_GAIN_PHYCTRL(0);
...
void RFReset()
{
    ...
    //REG32(0xCE000018) = 0x0055003C;  // bit 11 2013/12/27
    ...
    //REG32(0xCE004180) = 0x00044400;   // bit 0~1 2013/12/27
...
#define RF_SET_RG_PACKET_STAT_EN_11B(_VAL_)  \
      (ssv6xxx_drv_write_reg(0xce0023f8, (((_VAL_) << 20) | (ssv6xxx_drv_read_reg(0xce0023f8) & 0xffefffff))))
#define RF_SET_RG_PACKET_STAT_EN_11GN(_VAL_) \
      (ssv6xxx_drv_write_reg(0xce0043f8, (((_VAL_) << 20) | (ssv6xxx_drv_read_reg(0xce0043f8) & 0xffefffff))))

void RFReset()     // only reset count
{
    LOG_PRINTF("RFReset\r\n");

    RF_SET_RG_PACKET_STAT_EN_11GN(0);
    RF_SET_RG_PACKET_STAT_EN_11B(0);
    RF_SET_RG_PACKET_STAT_EN_11GN(1);
    RF_SET_RG_PACKET_STAT_EN_11B(1);
}
...
void RFTone(u16 value)
{
    u32 nCount = 0;
    u32 nCRC = 0;
    LOG_PRINTF("RFTone value = %d\r\n", value);

    // CE010000  0x5E00F00F
    ssv6xxx_drv_write_reg(0xCE010000,0x5e00f00f);

    // CA000408  0x00000003
    ssv6xxx_drv_write_reg(0xCA000408,0x00000003);

    // CE007044  0x02028080
    ssv6xxx_drv_write_reg(0xCE007044,0x02028080);

    // CE000000  0x80000014
    ssv6xxx_drv_write_reg(0xCE000000,0x80000014);

    // CE007000  0X00000000
    ssv6xxx_drv_write_reg(0xCE007000,0x00000000);

    // CE000084  0X10000000
    ssv6xxx_drv_write_reg(0xCE000084,0x10000000);

    if(value == 1)
    {
        ssv6xxx_drv_write_reg(0xCE007044,0x02026060);
    }
    else if(value == 2)
    {
        ssv6xxx_drv_write_reg(0xCE007044,0x02027272);
    }
    else if(value == 3)
    {
        ssv6xxx_drv_write_reg(0xCE007044,0x02028080);
...
int Parse_Radio_Command(s32 argc, char *argv[])
{
    ...
    if(argc==1){
        ...
        #else
        LOG_PRINTF("rf ch [value] \r\n");
        LOG_PRINTF("rf start \r\n");
        LOG_PRINTF("rf rate [value]\r\n");
        LOG_PRINTF("rf bgain [value]\r\n");
        LOG_PRINTF("rf gngain [value]\r\n");
        LOG_PRINTF("rf iqphase [value]\r\n");
        LOG_PRINTF("rf iqamp [value]\r\n");
        LOG_PRINTF("rf stop\r\n");
        LOG_PRINTF("rf reset \r\n");
        LOG_PRINTF("rf count [value] \r\n");
        LOG_PRINTF("rf tone [value] \r\n");
        #endif
        return -1;
    }
	...
	for(i=0; i<OP_NUM; i++)
	{
		if( !ssv6xxx_strncmp (argv[1], RADIO_OP[i], ssv6xxx_strlen(RADIO_OP[i]))   )
		{
			break;
		}
	}
