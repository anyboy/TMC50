...
#else
        LOG_PRINTF("[SOC_EVT_PS_WAKENED]: cli task no start\r\n");
#endif        
        break;
...
//gHCmdEngInfo
    ...
    if (strcmp(argv[1], "scan")==0) {
        ...
        */
        ...
        if (argc >= 5 )
            ssv6xxx_wifi_set_tx_duty((u32)ssv6xxx_atoi(argv[2]),(u32)ssv6xxx_atoi(argv[3]),(u32)ssv6xxx_atoi(argv[4]));
    }else if (strcmp(argv[1], "src")==0) {
        ssv6xxx_set_TXQ_SRC_limit((u32)ssv6xxx_atoi(argv[2]),(u32)ssv6xxx_atoi(argv[3]));
    }else if (strcmp(argv[1], "htq")==0) {
        ssv6xxx_halt_tx_queue((u32)ssv6xxx_atoi(argv[2]),(u32)ssv6xxx_atoi(argv[3]));
    }
	else {
...
void _cmd_wifi_scan (s32 argc, char *argv[])
{
    ...
    if (num_ssids > 0)
    {
        ...
        {
            if (strlen(argv[1 + i]) < MAX_SSID_LEN)
            {
                MEMCPY(ssids + i * MAX_SSID_LEN, argv[1 + i], strlen(argv[1 + i]) + 1);
            }
            else
            {
                MEMCPY(ssids + i * MAX_SSID_LEN, argv[1 + i], MAX_SSID_LEN);
            }
        }
...
void cmd_rc(s32 argc, char *argv[])
{
    ...
    else if(strcmp(argv[1], "txinfo")==0)
    {
        u16 tx_cnt,retry_cnt,phy_rate;
        ssv6xxx_get_tx_info(&tx_cnt, &retry_cnt, &phy_rate);
        LOG_PRINTF("phy_rate=%d,tx_cnt=%d, retry_cnt=%d!!\r\n",phy_rate,tx_cnt,retry_cnt);
        return;
    }
