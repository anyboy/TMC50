...
#define TWI_WRITE_ADDR                  (I2C_DEV0_ADDRESS)
...
#define I2C_DEV0_ADDRESS 	0x10//0x20  // 00100000B, LSB will be set to "1" (address = 0x21) when do read accessing
 ...
 *******************************************************************************/
 #define QND_INPUT_CLOCK_TYPE	QND_INPUT_CLOCK//QND_INPUT_CLOCK//
...
extern void QNF_SetRegBit(u8_t reg, u8_t bitMask, u8_t data_val) ;


extern u16_t QNF_GetCh(void) ;
#define QND_Delay(ms)      		k_busy_wait(ms * 1000)


extern int QN_ChipInitialization(void);

extern void QND_TuneToCH(u16_t ch) ;

extern void QNF_SetCh(u16_t freq);

extern void QND_RXSetTH(u8_t th) ;
extern int QND_RXValidCH(u16_t freq) ;
extern int QND_RXSeekCH(u16_t start, u16_t stop, u8_t step) ;
extern void QNF_SetMute(u8_t On) ;
