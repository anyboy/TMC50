...
#define __ASSERT(test, fmt, ...) {if (!(test)) { TRACE_ASSERT(_STRINGIFY(test),  __FILE__, __FUNCTION__,  __LINE__,  fmt, ##__VA_ARGS__);}}
#elif defined(CONFIG_ASSERT_SHOW_FILE)                 
#define __ASSERT(test, fmt, ...) {if (!(test)) { TRACE_ASSERT(_STRINGIFY(test),  __FILE__, __LINE__,  fmt, ##__VA_ARGS__);}}
#elif defined(CONFIG_ASSERT_SHOW_FUNC)                 
#define __ASSERT(test, fmt, ...) {if (!(test)) { TRACE_ASSERT(_STRINGIFY(test),  __FUNCTION__,  __LINE__,  fmt, ##__VA_ARGS__);}}
#else                                                  
#define __ASSERT(test, fmt, ...) {if (!(test)) { TRACE_ASSERT(_STRINGIFY(test),  __LINE__,  fmt, ##__VA_ARGS__);}}
#endif                                                 
