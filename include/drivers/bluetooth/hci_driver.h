...
/**
 * @brief Abstraction which represents the RAW HCI transport to the controller.
 *
 */
struct bt_raw_hci_driver {
	/** Name of the driver */
	const char *name;
	 ...
	 * @return 0 on success or negative error number on failure.
	 */
	int (*send)(u8_t *buf, u16_t len, u8_t type);
};

/**
 * @brief Receive raw hci data.
 *
 * @param buf hci data.
 * @param len buf data length
...
/**
 * @brief Register a new RAW HCI driver to the Bluetooth stack.
 *
 * This needs to be called before any application code runs.
 *
 * @param drv A bt_raw_hci_driver struct representing the driver.
 *
 * @return 0 on success or negative error number on failure.
 */
