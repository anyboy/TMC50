...
typedef u32_t (*dma_api_get_remain_size)(struct device *dev, u32_t channel);

typedef int (*dma_api_reload)(struct device *dev, u32_t channel, u32_t saddr, u32_t daddr, u32_t frame_len);

typedef int (*dma_api_request)(struct device *dev, u32_t channel);

typedef void (*dma_api_free)(struct device *dev, u32_t channel);

typedef int (*dma_api_wait_timeout)(struct device *dev, u32_t channel, u32_t timeout);

typedef int (*dma_api_prepare_start)(struct device *dev, u32_t channel);

typedef int (*dma_api_get_transfer_length)(struct device *dev, u32_t channel);

typedef int (*dma_api_get_phy_dma_channel)(struct device *dev, u32_t channel);
...
struct dma_driver_api {
	...
	dma_api_start start;
	dma_api_stop stop;
	dma_api_get_remain_size get_remain_size;
	dma_api_reload reload;
	dma_api_request request;
	dma_api_free free;
	dma_api_wait_timeout wait_timeout;
	dma_api_prepare_start prepare_start;
	dma_api_get_transfer_length get_transfer_length;
	dma_api_get_phy_dma_channel get_phy_dma_channel;
};
/**
 * @endcond
 */
...
/**
 * @brief Get the remain bytes of the transfer for DMA channel, the channel must be
 *        started beforehand.
 *
 * @param dev     Pointer to the device structure for the driver instance.
 * @param channel Numeric identification of the channel where the transfer will
 *                be processed
 *
 * @retval the number of remain bytes to be transferred for this transfer.
 */
static inline u32_t dma_get_remain_size(struct device *dev, u32_t channel)
{
	const struct dma_driver_api *api = dev->driver_api;

	return api->get_remain_size(dev, channel);
}

/**
 * @brief wait until the transfer finished for DMA channel.
 *
 * @param dev     Pointer to the device structure for the driver instance.
 * @param channel Numeric identification of the channel where the transfer will
 *                be processed
 ...
 * @retval N/A
 */
static inline void dma_wait_finished(struct device *dev, u32_t channel)
{
	const struct dma_driver_api *api = dev->driver_api;

	while (0 != api->get_remain_size(dev, channel))
		;
}
 ...
 * @param channel Numeric identification of the channel where the transfer will
 *                be processed
 * @param timeout timeout of dma wait
 *
 * @retval N/A
 */
static inline int dma_wait_timeout(struct device *dev, u32_t channel, u32_t timeout)
{
	const struct dma_driver_api *api = dev->driver_api;

    if(api->wait_timeout){
	    return api->wait_timeout(dev, channel, timeout);
	}else{
        return -ENOTSUP;
	}
}
...
/**
 * @brief config the DMA frame
 *
 * @param dev     Pointer to the device structure for the driver instance.
 * @param channel Numeric identification of the channel where the transfer was
 *                being processed
 * @param saddr source addr of dma
 * @param daddr dest addr of dma
 * @param frame_len frame len of data
 *
 * @retval 0 if successful.
 * @retval Negative errno code if failure.
 */
static inline int dma_reload(struct device *dev, u32_t channel, u32_t saddr, u32_t daddr, u32_t frame_len)
{
	const struct dma_driver_api *api = dev->driver_api;

	return api->reload(dev, channel, saddr, daddr, frame_len);
}
...
static inline void dma_free(struct device *dev, u32_t channel)
{
	const struct dma_driver_api *api = dev->driver_api;
...
/**
 * @brief Enables DMA channel and config the transfer, prepare to start.the channel must be
 *        configured beforehand.
 *
 * @param dev     Pointer to the device structure for the driver instance.
 * @param channel Numeric identification of the channel where the transfer will
 ...
 * @retval Negative errno code if failure.
 */
static inline int dma_prepare_start(struct device *dev, u32_t channel)
{
	const struct dma_driver_api *api = dev->driver_api;

	return api->prepare_start(dev, channel);
}

/**
 * @brief Get DMA channel current transfer length(byte unit)
 *        
 *
 * @param dev     Pointer to the device structure for the driver instance.
 * @param channel Numeric identification of the channel where the transfer will
 *                be processed
 *
 * @retval 0 transfer length
 * 
 */
static inline int dma_get_transfer_length(struct device *dev, u32_t channel)
{
	const struct dma_driver_api *api = dev->driver_api;

	return api->get_transfer_length(dev, channel);
}

/**
 * @brief Get DMA phy channel
 *        
 *
 * @param dev     Pointer to the device structure for the driver instance.
 * @param channel Numeric identification of the channel where the transfer will
 *                be processed
 *
 * @retval phy channel reg base addr
 * 
 */
static inline int dma_get_phy_channel(struct device *dev, u32_t channel)
{
	const struct dma_driver_api *api = dev->driver_api;

	return api->get_phy_dma_channel(dev, channel);
}
