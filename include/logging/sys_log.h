...
#ifdef CONFIG_SYS_LOG_SHOW_TAGS
/* [domain] [level] function: */
#define LOG_LAYOUT "%s: %s"
#define LOG_BACKEND_CALL(level, log_lv, log_color, log_format, color_off, ...)	\
	do {	\
		if(syslog_log_level >= level) \
			...
			__func__, log_color, ##__VA_ARGS__, color_off); \
	} while (0)

#define LOG_NO_COLOR_LAYOUT "%s: "
#define LOG_NO_COLOR_BACKEND_CALL(level, log_lv, log_format, ...)	\
	do {	\
		if(syslog_log_level >= level) \
			SYS_LOG_BACKEND_FN(LOG_NO_COLOR_LAYOUT log_format SYS_LOG_NL,	\
...
#define LOG_NO_COLOR(level, log_lv, log_format, ...)				\
	LOG_NO_COLOR_BACKEND_CALL(level, log_lv, log_format, ##__VA_ARGS__)
...
#define SYS_LOG_ERR(...) LOG_COLOR(SYS_LOG_LEVEL_ERROR,SYS_LOG_TAG_ERR, SYS_LOG_COLOR_RED,	\
	##__VA_ARGS__)
#else
#define SYS_LOG_ERR(...) LOG_NO_COLOR(SYS_LOG_LEVEL_ERROR,SYS_LOG_TAG_ERR, ##__VA_ARGS__)
#endif
...
#else
#define SYS_LOG_WRN(...) LOG_NO_COLOR(SYS_LOG_LEVEL_WARNING,SYS_LOG_TAG_WRN, ##__VA_ARGS__)
#endif
#endif
