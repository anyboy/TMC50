...
struct hid_ops {
	...
	/*
	 * int_out_ready is an optional callback that is called when
	 * the current interrupt OUT transfer has completed.  This can
	 * be used to wait for the endpoint to go idle or to trigger
	 * the next transfer.
	 */
...
/* Read hid interrupt endpoint */
int hid_int_ep_read(u8_t *data, u32_t data_len, u32_t *bytes_ret);

/* HID TX Self-define data */
int hid_tx_self_define(const u8_t *buf, u16_t len);

int hid_in_ep_flush(void);

int hid_out_ep_flush(void);

/* Initialize USB HID devie */
int usb_hid_init(void);

/* deinitialize USB HID device */
int usb_hid_exit(void);

/* Initialize USB HID composite devie */
int usb_hid_composite_init(void);

/* Initialize Shell Subsys */
int shell_subsys_init(void);
