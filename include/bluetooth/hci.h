...
#define BT_HCI_ERR_CONNECTION_LIMIT_EXCEEDED    0x09
#define BT_HCI_ERR_SYN_CONNECTION_LIMIT_DEVICE  0x0a
#define BT_HCI_ERR_ACL_CONNECTION_ALREADY_EXIT  0x0b
...
#define BT_HCI_ERR_REMOTE_TERM_CONN_DUO_TO_POWEROFF 0x15
#define BT_HCI_ERR_LOCAL_USER_TERM_CONN         0x16
...
#define BT_DATA_DEVICE_ID               0x10 /* Device ID */
...
#define BT_HCI_OP_ADD_SCO_CONN               BT_OP(BT_OGF_LINK_CTRL, 0x0007)
struct bt_hci_cp_add_sco_conn {
	u16_t  handle;
	u16_t  pkt_type;
} __packed;
...
#define BT_HCI_OP_SET_CSB_BROADCAST             BT_OP(BT_OGF_LINK_CTRL, 0x0041)
struct bt_hci_cp_set_csb_broadcast {
	u8_t enable;
	u8_t lt_addr;
	u8_t lpo_allowed;
	u16_t packet_type;
...
#define BT_HCI_OP_SET_CSB_RECEIVE              BT_OP(BT_OGF_LINK_CTRL, 0x0042)
struct bt_hci_cp_set_csb_receive {
	u8_t enable;
	u8_t bd_addr[6];
	u8_t lt_addr;
	u16_t interval;
	u32_t clock_offset;
	u32_t next_csb_clock;
	u16_t csb_supervisionTO;
	u8_t remote_timing_accuracy;
	u8_t skip;
	u16_t packet_type;
	u8_t afh_channel_map[10];
} __packed;

#define BT_HCI_OP_START_SYNC_TRAIN              BT_OP(BT_OGF_LINK_CTRL, 0x0043)

#define BT_HCI_OP_RECEIVE_SYNC_TRAIN            BT_OP(BT_OGF_LINK_CTRL, 0x0044)
struct bt_hci_cp_receive_sync_train {
	u8_t bd_addr[6];
	u16_t sync_scan_timeout;
	u16_t sync_scan_windown;
	u16_t sync_scan_interval;
} __packed;
...
#define BT_HCI_OP_ROLE_DISCOVERY                BT_OP(BT_OGF_LINK_POLICY, 0x0009)
...
struct bt_hci_cp_wd_link_policy {
	u16_t   link_policy;
} __packed;
...
#define BT_HCI_OP_EVENT_FILTER                  BT_OP(BT_OGF_BASEBAND, 0x0005)
...
struct bt_hci_write_clase_of_device {
	u8_t class_of_device[3];
} __packed;
...
struct bt_hci_cp_write_current_iac_lap {
	u8_t num_current_iac;
	u8_t iac_lap[3];	/* Current only support 1 iac */
} __packed;
...
struct bt_hci_cp_write_mode_type {
	u8_t  mode_type;
} __packed;

#define BT_HCI_OP_WRITE_EXTENDED_INQUIRY_RSP    BT_OP(BT_OGF_BASEBAND, 0x0052)
struct bt_hci_cp_write_eir {
	u8_t fcc;
	u8_t eirdata[240];
} __packed;
...
#define BT_HCI_OP_SET_RESERVED_LT_ADDR          BT_OP(BT_OGF_BASEBAND, 0x0074)
struct bt_hci_cp_set_reserved_lt_addr {
	u8_t  lt_addr;
} __packed;

#define BT_HCI_OP_SET_CSB_DATA                  BT_OP(BT_OGF_BASEBAND, 0x0076)
#define CSB_SET_DATA_LEN                        (255 - 18)
struct bt_hci_cp_set_csb_data {
	u8_t lt_addr;
	u8_t fragmemt;
	u8_t data_len;
	u8_t data[0];
...
enum {
	CSB_Continuation_fragment,
	CSB_Starting_fragment,
	CSB_Ending_fragment,
	CSB_Single_fragment,
};

#define BT_HCI_OP_WRITE_SYNC_TRAIN_PARAM        BT_OP(BT_OGF_BASEBAND, 0x0078)
struct bt_hci_cp_write_sync_train_param {
	u16_t  interval_min;
	u16_t  interval_max;
	u32_t sync_trainto;
	u8_t service_data;
...
/* TEST */
#define BT_HCI_OP_TEST_ENABLE_UNDER_TEST        BT_OP(BT_OGF_TEST, 0x0003)

#define BT_HCI_OP_CSB_LINK_KEY_SEED             BT_OP(BT_OGF_TEST, 0x0005)
struct bt_hci_cp_csb_link_key_seed {
	u8_t link_key_seed[16];
} __packed;
...
#define BT_HCI_EVT_SYNC_TRAIN_RECEIVE           0x50
struct bt_hci_evt_sync_train_receive {
	u8_t  status;
	u8_t bd_addr[6];
	u32_t clock_offset;
	u8_t afh_channel_map[10];
	u8_t lt_addr;
	u32_t next_broadcast_instant;
	u16_t csb_interval;
	u8_t service_data;
} __packed;

#define BT_HCI_EVT_CSB_RECEIVE                  0x51
struct bt_hci_evt_csb_receive {
	u8_t bd_addr[6];
	u8_t lt_addr;
	u32_t clk;
	u32_t offset;
...
#define BT_HCI_EVT_CSB_TIMEOUT                  0x52
struct bt_hci_evt_csb_timeout {
	u8_t bd_addr[6];
	u8_t lt_addr;
} __packed;
