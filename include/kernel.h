...
/**
 * @brief Calculate element in a queue.
 */
extern int k_queue_cnt_sum(struct k_queue *queue);
...
#define k_lifo_remove(lifo, data) \
	k_queue_remove((struct k_queue *)lifo, data)
...
enum {
	...
	K_WORK_STATE_RUNNING,	/* Work item running state */
};
...
/**
 * @brief Check if a work item is running.
 *
 * This routine indicates if work item @a work is running in a workqueue's
 * queue.
 *
 * @note Can be called by ISRs.
 *
 * @param work Address of work item.
 *
 * @return 1 if work item is running, or 0 if it is not running.
 */
static inline int k_work_running(struct k_work *work)
{
	return atomic_test_bit(work->flags, K_WORK_STATE_RUNNING);
}
...
/**
 * @brief Cancel a delayed work item and wait for it to finish.
 *
 * This routine cancels the submission of delayed work item @a work.
 * If this delayed work is busy, wait ultil it finished or timed out.
 *
 * @note Can be called by ISRs only if timeout is K_NO_WAIT.
 *
 * @param work Address of delayed work item.
 * @param timeout timeout of cancel.
 *
 * @retval 0 Work item countdown canceled.
 * @retval -EBUSY Work cancel operation times out.
 * @retval -EINVAL Work item is being processed or has completed its work.
 */
extern int k_delayed_work_cancel_sync(struct k_delayed_work *work,
				      s32_t timeout);
