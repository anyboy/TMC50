 ...
 */
int fs_disk_detect(const char *str, uint8_t *state);

/**
 * @brief Mount disk
 *
 * Init the file system volume for related disk such as USB.
 *
 * @param fs File system object structure (FATFS)
 * @param str One of _VOLUME_STRS or numeric drive id with colon
 *
 * @retval 0 Success
 * @retval -ERRNO errno code if error
 ...
 * @brief Unmount disk
 *
 * Uninit the file system volume for related disk such as USB.
 *
 * @param str One of _VOLUME_STRS or numeric drive id with colon
 *
 * @retval 0 Success
...
/**
 * @brief Change the size of an open file(expand quickly)
 *
 * Truncates just like fs_truncate(). When expands the file, only sync
 * instead of filling the whole expanded region with zeroes which will
 * take much less time.
 *
 * @param zfp Pointer to the file object
 * @param length New size of the file in bytes
 ...
 *
 * @param vol_name Pointer to volume name
