 ...
 * @param data_pool point of data pool
 */
...
struct net_buf *net_pkt_get_inner_debug(struct net_context *context,
				       s32_t timeout,
				       const char *caller, int line);
#define net_pkt_get_inner(context, timeout)				\
	net_pkt_get_inner_debug(context, timeout, __func__, __LINE__)
#endif
...
struct net_buf *net_pkt_get_reserve_inner_data_debug(u16_t reserve_head,
						  ...
						  const char *caller,
						  int line);
#define net_pkt_get_reserve_inner_data(res, timeout)			\
	net_pkt_get_reserve_inner_data_debug(res, timeout, __func__, __LINE__)
#endif
...
#if defined(CONFIG_NET_NBUF_INNER)
struct net_buf *net_pkt_get_inner(struct net_context *context,
				 s32_t timeout);
#endif
...
/**
 * @brief Get inner DATA buffer from pool but also reserve headroom for
* potential headers.
*/
#if defined(CONFIG_NET_NBUF_INNER)
...
void net_pktbuf_set_caller(void *alloc_data, const char *caller,
				  int line);
#define net_pktbuf_set_owner(alloc_data)	net_pktbuf_set_caller(alloc_data, __func__, __LINE__)

#else
#define net_pkt_print(...)
#define net_pkt_print_info(...)
#define net_pktbuf_set_owner(...)
#endif /* CONFIG_NET_DEBUG_NET_PKT */
...
#else
static inline bool net_nbuf_add_dynamic_frag(void)
{
	return false;
}

static inline bool net_nbuf_release_dynamic_frag(void)
{
	return false;
}
#endif
