...
struct net_buf {
	 ...
	 */
	 u8_t user_data[0] __net_buf_align;
};

/* Net buf data pool */
struct data_pool_opt {
	void (*init)(void *data_pool);
	u8_t *(*malloc)(struct net_buf *buf, u16_t size, s32_t timeout);
	void (*free)(struct net_buf *buf);
};

struct net_data_pool {
	u16_t data_size;				/* Max size 65535 */
	u8_t *data_pool;
	struct data_pool_opt *opt;

	union {
		/* Sequential memory allocation manager */
		struct {
			u8_t *alloc_pos;
		};
		/* Wait TODO: Block manager */
		struct {
			u32_t count;
		};
	};
...
static inline void net_data_pool_reinitializer(struct net_data_pool *pool)
{
	if (pool->opt && pool->opt->init) {
		pool->opt->init(pool);
	}
}

#define NET_DATA_POOL_INITIALIZER(_mem_name, _count, _total_size, _opt) \
		{ \
...
#define NET_DATA_POOL_DEFINE(_name, _count, _size, _opt)	\
	static u8_t _net_data_##_name[ROUND_UP(((_count)*(_size)), 4)]; \
	struct net_data_pool _name __net_buf_align = \
		NET_DATA_POOL_INITIALIZER(_net_data_##_name, _count, ROUND_UP(((_count)*(_size)), 4), _opt)


#define BT_DATA_POOL_DEFINE(_name, _count, _size, _opt)	\
	static u8_t _net_data_##_name[ROUND_UP(((_count)*(_size)), 4)] __net_buf_align __in_section(_bt_buf_pool, noinit, _name); \
	struct net_data_pool _name __net_buf_align = \
		NET_DATA_POOL_INITIALIZER(_net_data_##_name, _count, ROUND_UP(((_count)*(_size)), 4), _opt)
...
struct net_buf_pool {
	...
	const u16_t user_data_size:12;

	/** For pool flag used  */
	const u16_t use_data_pool:1;			/* Use user data pool malloc buf */
	const u16_t reserve:3;

	/** Where the data buffer malloc/free  */
	struct net_data_pool *data_pool;

#if defined(CONFIG_NET_BUF_POOL_USAGE)
	/** Amount of available buffers in the pool. */
	s16_t avail_count;

	/** Amount of maximize buffers in the pool. */
	u16_t max_used;
...
#define NET_BUF_POOL_INITIALIZER(_pool, _bufs, _count, _size, _ud_size,      \
				 _destroy, _use_data_pool, _data_pool)               \
	{                                                                \
		...
		.use_data_pool = _use_data_pool,                             \
		.data_pool = _data_pool,                                     \
		.destroy = _destroy,                                         \
		.name = STRINGIFY(_pool),                                    \
	}

static inline void net_buf_pool_reinitializer(struct net_buf_pool *pool, u16_t count)
{
	 k_lifo_init(&pool->free);
	 pool->uninit_count = count;
	 pool->avail_count = count;
	 pool->max_used = 0;
}
 ...
 *  @param _data_pool point of data pool	
 */
#define NET_BUF_POOL_DEFINE(_name, _count, _size, _ud_size, _destroy)        \
	...
	struct net_buf_pool _name __net_buf_align                            \
		...
		NET_BUF_POOL_INITIALIZER(_name, _net_buf_##_name,            \
					 _count, _size, _ud_size, _destroy, 0, NULL); \

#define BT_BUF_POOL_DEFINE(_name, _count, _size, _ud_size, _destroy, _data_pool)        \
	static struct {                                                      \
		struct net_buf buf;                                          \
		u8_t ud[ROUND_UP(_ud_size, 4)] __net_buf_align;              \
	} _net_buf_##_name[_count] __in_section(_bt_buf_pool, noinit, _name); \
	struct net_buf_pool _name __net_buf_align                            \
			__in_section(_net_buf_pool, static, _name) =         \
		NET_BUF_POOL_INITIALIZER(_name, _net_buf_##_name,            \
					 _count, _size, _ud_size, _destroy, 1, _data_pool); \
...
/**
 *  @brief Allocate a new buffer from a data pool.
 *
 *  Allocate a new buffer from a data pool.
 *
 *  @param pool Which pool to allocate the buffer from.
 *  @param len alloc buf length
 *  @param timeout Affects the action taken should the pool be empty.
 *         If K_NO_WAIT, then return immediately. If K_FOREVER, then
 *         wait as long as necessary. Otherwise, wait up to the specified
 *         number of milliseconds before timing out.
...
struct net_buf *net_buf_alloc_len_debug(struct net_buf_pool *pool, u16_t len, s32_t timeout,
				    const char *func, int line);
#define	net_buf_alloc_len(_pool, len, _timeout) \
	net_buf_alloc_len_debug(_pool, len, _timeout, __func__, __LINE__)
#else
...
/** @brief get net buf pool id from pool.
 *
 *  @param pool net buf pool pointer
 *
 *  @return poo id.
 */
int net_buf_pool_id(struct net_buf_pool *pool);

/* Data pool opration function struct */
extern struct data_pool_opt continue_data_pool_opt;
