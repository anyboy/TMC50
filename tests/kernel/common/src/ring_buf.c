...
void ring_buffer_test(void)
{
	...
	put_count = 0;
	while (1) {
		ret = ring_buf_item_put(&ring_buf, TYPE, VALUE,
				       (u32_t *)data, dsize);
		if (ret == -EMSGSIZE) {
	...
	ret = ring_buf_item_get(&ring_buf, &gettype, &getval, getdata, &getsize);
	if (ret != -EMSGSIZE) {
	...
	for (i = 0; i < put_count; i++) {
		...
		ret = ring_buf_item_get(&ring_buf, &gettype, &getval, getdata,
				       &getsize);
