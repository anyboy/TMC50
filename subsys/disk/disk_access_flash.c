...
static int flash_disk_access_status(struct disk_info *disk)
{
	if (!flash_dev) {
...
static int flash_disk_access_init(struct disk_info *disk)
{
	if (flash_dev) {
		return 0;
	}
...
static int flash_disk_access_read(struct disk_info *disk, u8_t *buff, u32_t start_sector,
		     u32_t sector_count)
{
	off_t fl_addr;
	u32_t remaining;
	...
	limit = (CONFIG_DISK_FLASH_START + CONFIG_DISK_VOLUME_SIZE) / SECTOR_SIZE;
...
static int flash_disk_access_write(struct disk_info *disk, const u8_t *buff, u32_t start_sector,
		      u32_t sector_count)
{
	off_t fl_addr;
	u32_t remaining;
...
static int flash_disk_access_ioctl(struct disk_info *disk, u8_t cmd, void *buff)
{
	switch (cmd) {
...
static const struct disk_operation disk_flash_operation = {
	.init		= flash_disk_access_init,
	.get_status	= flash_disk_access_status,
	.read		= flash_disk_access_read,
	.write		= flash_disk_access_write,
	.ioctl		= flash_disk_access_ioctl,
};

static const struct disk_info disk_flash = {
	.name		= "FLASH",
	.id		= 0,
	.flag		= 0,
	.sector_size	= SECTOR_SIZE,
	.sector_offset	= CONFIG_DISK_FLASH_START / SECTOR_SIZE,
	.sector_cnt	= CONFIG_DISK_VOLUME_SIZE / SECTOR_SIZE,
	.op		= &disk_flash_operation,
};

static int flash_disk_init(struct device *dev)
{
	disk_register((struct disk_info *)&disk_flash);

	return 0;
}

SYS_INIT(flash_disk_init, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
