...
int net_shell_cmd_mem(int argc, char *argv[])
{
	...
	if ((argc > 1) && (!strcmp(argv[1], "debug"))) {
		net_pkt_print_info(1);
	} else {
		net_pkt_print_info(0);
	}
#endif
...
#if defined(DEBUG_NET_SPEED)

#define NETLOAD_CHECK_INTERVAL	1000		/* 1s */
static u32_t net_received = 0;
static u32_t net_sent = 0;
static u8_t	check_tcp_speed = 0;

static void netload_stat_timer_callback(struct k_timer *timer);
K_TIMER_DEFINE(netload_timer, netload_stat_timer_callback, NULL);

static void speed_tcp_cb(struct net_tcp *tcp, void *user_data)
{
	printk("Sport:%u\tDport:%u\tTx:%d\tRx:%d\tTx:%d kbps\tRx:%d kbps\n",
	       ntohs(net_sin6_ptr(&tcp->context->local)->sin6_port),
	       ntohs(net_sin6(&tcp->context->remote)->sin6_port),
	       tcp->send_len, tcp->receive_len,
	       (tcp->send_len - tcp->pre_send_len)*8/1024,
		   (tcp->receive_len - tcp->pre_receive_len)*8/1024);

	tcp->pre_receive_len = tcp->receive_len;
	tcp->pre_send_len = tcp->send_len;
}

static void netload_stat_timer_callback(struct k_timer *timer)
{
	if (check_tcp_speed == 0) {
		printk("Speed TX:%d\tRX:%d\tTX:%d kbps\tRX:%d kbps\n",
				GET_STAT(bytes.sent), GET_STAT(bytes.received),
				(GET_STAT(bytes.sent) - net_sent)*8/1024,
				(GET_STAT(bytes.received) - net_received)*8/1024);

		net_received = GET_STAT(bytes.received);
		net_sent = GET_STAT(bytes.sent);
	} else {
#if defined(CONFIG_NET_TCP)
		net_tcp_foreach(speed_tcp_cb, NULL);
#endif
	}
}

static int shell_cmd_speed(int argc, char *argv[])
{
#if defined(CONFIG_NET_TCP)
#if defined(CONFIG_NET_STATISTICS)
	if (!strcmp(argv[1], "start")) {
		...
		printk("Start net load statistic, interval %d ms\n", NETLOAD_CHECK_INTERVAL);
		k_timer_start(&netload_timer, NETLOAD_CHECK_INTERVAL, NETLOAD_CHECK_INTERVAL);
	} else if (!strcmp(argv[1], "stop")) {
		printk("Stop net load statistic\n");
		k_timer_stop(&netload_timer);
	} else {
		printk("usage:\n");
		printk("  net speed start [tcp]\n");
		printk("  net speed stop\n");
...
extern void mqtt_servers_tcpdump(bool start, bool net_opt);
extern void tcpdump_console_output(void);

static int shell_cmd_tcpdump(int argc, char *argv[])
{
	if (!strcmp(argv[1], "start")) {
		mqtt_servers_tcpdump(true, false);
	} else if (!strcmp(argv[1], "stop")) {
		mqtt_servers_tcpdump(false, false);
	} else if (!strcmp(argv[1], "output")) {
		tcpdump_console_output();
	}
	return 0;
}
#endif
...
#if defined(DEBUG_NET_SPEED)
	{ "speed", shell_cmd_speed, "Test net speed" },
#endif
#ifdef CONFIG_REMOTE_DEBUG
	{ "tcpdump", shell_cmd_tcpdump, NULL },
#endif
	{ NULL, NULL, NULL }
};
