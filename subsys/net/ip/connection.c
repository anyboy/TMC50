...
static int net_conn_unregister_prio(struct net_conn_handle *handle)
{
...
int net_conn_unregister(struct net_conn_handle *handle)
{
	...
	ret = net_conn_unregister_prio(handle);
...
static int net_conn_register_prio(enum net_ip_protocol proto,
		      const struct sockaddr *remote_addr,
		      const struct sockaddr *local_addr,
		      u16_t remote_port,
		      u16_t local_port,
		      ...
		      struct net_conn_handle **handle)
{
	...
	ret = net_conn_register_prio(proto, remote_addr, local_addr, remote_port,
				local_port, cb, user_data, handle);
