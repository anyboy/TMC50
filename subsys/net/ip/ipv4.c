				    ...
				    u8_t next_header)
{
	...
	header = net_pkt_get_reserve_inner_data(net_pkt_ll_reserve(pkt), K_FOREVER);
#else
...
enum net_verdict net_ipv4_process_pkt(struct net_pkt *pkt)
{
	...
	if (real_len != pkt_len) {
		...
		u8_t *ipdata = net_pkt_ip_data(pkt) + sizeof(struct net_ipv4_hdr);

		src_port = (ipdata[0] << 8) | ipdata[1];
		dst_port = (ipdata[2] << 8) | ipdata[3];

		if ((real_len > pkt_len) && (hdr->proto == IPPROTO_UDP) &&
			(src_port == DHCPV4_SERVER_PORT) && (dst_port == DHCPV4_CLIENT_PORT)) {
			/* UDP packet, dhcpv4, some router send packet with uncorrect lengh */
		} else {
