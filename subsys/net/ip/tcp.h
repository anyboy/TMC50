...
/* TCP max window size */
#define NET_TCP_MAX_WIN   CONFIG_NET_TCP_WINDOW_SIZE
...
struct net_tcp {
	...
	/** timer run in interrupt thread, better run retransmit in work */
	struct k_work retry_work;
	u32_t retry_work_start;
	...
	/** Record tcp rx data len */
	uint32_t receive_len;
	uint32_t pre_receive_len;

	/** Record tcp tx data len */
	uint32_t send_len;
	uint32_t pre_send_len;

	/* for keep packet */
	struct net_pkt *keep_pkt[NET_TCP_MAX_KPKT];
	u32_t keep_pkt_seq[NET_TCP_MAX_KPKT];
};
