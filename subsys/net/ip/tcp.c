...
#endif
#else
#define CONFIG_DNS_RESOLVER_MAX_SERVERS 0
#endif

#define NET_MAX_TCP_CONTEXT (CONFIG_NET_MAX_CONTEXTS - CONFIG_DNS_RESOLVER_MAX_SERVERS)
...
static void do_tcp_retry_work(struct k_work *work)
{
	struct net_tcp *tcp = CONTAINER_OF(work, struct net_tcp, retry_work);
	struct net_pkt *pkt;

	if (!tcp->retry_work_start) {
		return;
	}
	...
	if (!sys_slist_is_empty(&tcp->sent_list)) {
		...
		if (net_pkt_sent(pkt)) {
			...
			/* The pkt still in tx_queue wait to send, no need send agian
			 * If send without do ref, will case unexpected unref pkt.
			 */
			return;
		}
...
static void tcp_retry_expired(struct k_timer *timer)
{
	...
	tcp->retry_work_start = true;
	k_work_submit_to_queue(&k_sys_work_q, &tcp->retry_work);
}
...
struct net_tcp *net_tcp_alloc(struct net_context *context)
{
	...
	for (j=0; j<NET_TCP_MAX_KPKT; j++) {
		tcp_context[i].keep_pkt[j] = NULL;
	}

	tcp_context[i].receive_len = 0;
	tcp_context[i].pre_receive_len = 0;
	tcp_context[i].send_len = 0;
	tcp_context[i].pre_send_len = 0;

	tcp_context[i].ctrl_ack = 0;

	k_timer_init(&tcp_context[i].retry_timer, tcp_retry_expired, NULL);
	k_work_init(&tcp_context[i].retry_work, do_tcp_retry_work);
	tcp_context[i].retry_work_start = false;
...
int net_tcp_release(struct net_tcp *tcp)
{
	...
	for (i = 0; i < NET_TCP_MAX_KPKT; i++) {
		if (tcp->keep_pkt[i]) {
			net_pkt_unref(tcp->keep_pkt[i]);
			tcp->keep_pkt[0] = NULL;
		}
	}

	k_timer_stop(&tcp->retry_timer);
	tcp->retry_work_start = false;
				       ...
				       struct net_pkt *pkt)
{
	...
	header = net_pkt_get_inner(context, ALLOC_TIMEOUT);
#else
...
/* Avoid one app used all tx pkt */
#define NET_TCP_MAX_TX_KEEP		(CONFIG_NET_PKT_TX_COUNT - 1)

int net_tcp_queue_data(struct net_context *context, struct net_pkt *pkt)
{
	struct net_conn *conn = (struct net_conn *)context->conn_handler;
	size_t data_len = net_pkt_get_len(pkt);
	int ret, wait_to_send;
	s32_t remain_time;
	struct net_pkt *buflist;

	NET_DBG("[%p] Queue %p len %zd", context->tcp, pkt, data_len);

	wait_to_send = 0;
	SYS_SLIST_FOR_EACH_CONTAINER(&context->tcp->sent_list, buflist,
						     sent_list) {
		wait_to_send++;
	}

	/* warning: limit buf, can't keep to much send buf */
	if (wait_to_send >= NET_TCP_MAX_TX_KEEP) {
		return -EAGAIN;
	}
	...
	remain_time = k_timer_remaining_get(&context->tcp->retry_timer);
	if (remain_time == 0) {
		k_timer_start(&context->tcp->retry_timer,
			      retry_timeout(context->tcp), 0);
	} else if (remain_time > 1000) {
		/* tcp retry send timeout more than 1s,
		 * and have next packet to send
		 * restart timeout to accelerate resend
		 */
		context->tcp->retry_timeout_shift = 0;
	}
...
void net_tcp_ack_received(struct net_context *ctx, u32_t ack)
{
	...
	while (!sys_slist_is_empty(list)) {
		...
		tcp->send_len += net_pkt_appdatalen(pkt);
		sys_slist_remove(list, NULL, head);
		net_pkt_unref(pkt);
		valid_ack = true;
	}
