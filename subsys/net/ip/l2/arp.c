...
#define ARP_PENDING_KEEP_TIME			(5*1000)		/* 5s */
			      ...
			      struct net_eth_addr *hwaddr)
{
	...
	for (i = 0; i < CONFIG_NET_ARP_TABLE_SIZE; i++) {
		...
		/* Release dead pending arp_entry, avoid no arp arp_entry for used */
		if (arp_table[i].pending && (!net_ipv4_addr_cmp(&arp_table[i].ip, src)) &&
			((k_uptime_get_32() - arp_table[i].time) > ARP_PENDING_KEEP_TIME)) {
