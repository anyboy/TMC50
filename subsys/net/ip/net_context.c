...
#if defined(USED_FAST_ACK)
#define FAST_ACK_TIMER_INTERVAL		300			/* 300ms */

static struct k_work fast_ack_work;

static void fast_ack_timer_callback(struct k_timer *timer);
K_TIMER_DEFINE(fast_ack_timer, fast_ack_timer_callback, NULL);
#endif
...
#define TCP_MAX_MSS		((min(NET_TCP_MAX_WIN, NET_TCP_BUF_MAX_LEN) \
						> (1460*2)) ? 1460 : \
						((min(NET_TCP_MAX_WIN, NET_TCP_BUF_MAX_LEN)/2)))
				       ...
				       int flags, const char *msg)
{
	struct net_pkt *pkt = NULL;
	int ret;
	char options[4] = { 2, NET_TCP_MSS_SIZE, ((TCP_MAX_MSS&0xFF00) >> 8), (TCP_MAX_MSS&0xFF)};

	if (flags & NET_TCP_SYN) {
		ret = net_tcp_prepare_segment(context->tcp, flags, options, 4,
			      local, remote, &pkt);
	} else {
...
static bool tcp_check_keep_pkt(struct net_context *context, struct net_conn *conn, int index)
{
	struct net_tcp_hdr hdr, *tcp_hdr;
	...
	pkt = context->tcp->keep_pkt[index];
	context->tcp->keep_pkt[index] = NULL;
	...
	/* No need process ACK and RST flag, it had process in NET_CONN_CB(tcp_established) */

	if (net_tcp_seq_cmp(sys_get_be32(tcp_hdr->seq),
			    context->tcp->send_ack) < 0) {
		/* Peer sent us packet we've already seen. Apparently */
		/* Keep packet, no need send ack again */
		net_pkt_unref(pkt);
		return false;
	}

	if (sys_get_be32(tcp_hdr->seq) - context->tcp->send_ack) {
		/* If it doesn't match the next segment exactly,
		 *  still need keep for future
		 */
		context->tcp->keep_pkt[index] = pkt;
		return false;
	}
	...
	context->tcp->receive_len += data_len;
	...
	if ((!context->tcp->ctrl_ack) || (tcp_flags & NET_TCP_FIN)) {
...
static void retry_keep_pkt(struct net_context *context, struct net_conn *conn)
{
	bool need_retry = false;
	int i;

	for (i=0; i<NET_TCP_MAX_KPKT; i++) {
		NET_ASSERT(context && context->tcp);

		if (net_tcp_get_state(context->tcp) < NET_TCP_ESTABLISHED) {
			NET_ERR("Not in NET_TCP_ESTABLISHED state\n");
			return;
		}

		if (context->tcp->keep_pkt[i]) {
			need_retry = tcp_check_keep_pkt(context, conn, i);
		}

		if (need_retry) {
			/* tcp->send_ack had change, try again
			* for() will do i++, so set i to -1, this begin from 0
			*/
			i = -1;
			need_retry = false;
		}
...
static enum net_verdict pkt_keep_or_drop(struct net_pkt *pkt,
					u32_t seq, struct net_context *context)
{
	int i, d;

	/* come to here, NET_TCP_BUF(buf)->seq - context->tcp->send_ack,
	  *  is positive and less than 2^29
	  */
	d = (int)(seq - context->tcp->send_ack);

	if (d <= min(NET_TCP_MAX_WIN, NET_TCP_BUF_MAX_LEN)) {
		for (i=0; i<NET_TCP_MAX_KPKT; i++) {
			if ((context->tcp->keep_pkt[i] != NULL) &&
				 (context->tcp->keep_pkt_seq[i] == seq)) {
				/* the seq no pkt already in keep pkt, just drop */
				return NET_DROP;
			}
		}
		...
		for (i=0; i<NET_TCP_MAX_KPKT; i++) {
			if (context->tcp->keep_pkt[i] == NULL) {
				context->tcp->keep_pkt[i] = pkt;
				context->tcp->keep_pkt_seq[i] = seq;
				return NET_OK;
			}
		}

		SYS_LOG_INF("Warning: not enough keep_pkt, drop pkt\n");
	}
...
NET_CONN_CB(tcp_established)
{
	...
	if (net_tcp_seq_cmp(sys_get_be32(tcp_hdr->seq),
		 ...
		 */
		if (!context->tcp->ctrl_ack || (context->tcp->send_ack == context->tcp->sent_ack)) {
...
#if defined(USED_FAST_ACK)
		send_ack(context, &conn->remote_addr, true);
		if (k_timer_remaining_get(&fast_ack_timer) == 0) {
			k_timer_start(&fast_ack_timer, FAST_ACK_TIMER_INTERVAL,
							FAST_ACK_TIMER_INTERVAL);
		}
#endif
		return pkt_keep_or_drop(pkt, sys_get_be32(tcp_hdr->seq), context);
	}
		  ...
		  void *user_data)
{
	...
	if (!net_context_is_used(context) || (pkt->frags == NULL)) {
		return -EBADF;
	}
...
static int sendto_tcpip(struct net_pkt *pkt,
		  const struct sockaddr *dst_addr,
		  socklen_t addrlen,
		  net_context_send_cb_t cb,
		  s32_t timeout,
		  ...
		  void *user_data)
{
	...
	ret = sendto(pkt, dst_addr, addrlen, cb, timeout, token, user_data);
		     ...
		     void *user_data)
{
	...
	return sendto_tcpip(pkt, &context->remote, addrlen, cb, timeout, token,
		      user_data);
}
		       ...
		       void *user_data)
{
	...
	return sendto_tcpip(pkt, dst_addr, addrlen, cb, timeout, token, user_data);
}
...
#if defined(USED_FAST_ACK)
static void tcp_fast_ack_work(struct k_work *item)
{
	u8_t i, j;
	struct net_tcp *tcp;
	struct net_conn *conn;
	static u8_t fast_ack_context_pos = 0;
	...
	for (i = 0; i < NET_MAX_CONTEXT; i++) {
		if (fast_ack_context_pos == NET_MAX_CONTEXT) {
			fast_ack_context_pos = 0;
		}

		if (!net_context_is_used(&contexts[fast_ack_context_pos]) ||
			(contexts[fast_ack_context_pos].tcp == NULL)) {
			fast_ack_context_pos++;
			continue;
		}

		tcp = contexts[fast_ack_context_pos].tcp;
		if (net_tcp_get_state(tcp) != NET_TCP_ESTABLISHED) {
			fast_ack_context_pos++;
			continue;
		}

		for (j=0; j<NET_TCP_MAX_KPKT; j++) {
			if (tcp->keep_pkt[j] != NULL) {
				break;
			}
		}

		if (j == NET_TCP_MAX_KPKT) {
			/* Not fast ack to send */
			fast_ack_context_pos++;
			continue;
		}

		conn = (struct net_conn *)contexts[fast_ack_context_pos].conn_handler;
		if (!contexts[fast_ack_context_pos].tcp->ctrl_ack ||
			(contexts[fast_ack_context_pos].tcp->send_ack == contexts[fast_ack_context_pos].tcp->sent_ack)) {
			send_ack(&contexts[fast_ack_context_pos], &conn->remote_addr, true);
		}
		fast_ack_context_pos++;
		break;
	}

	if (i == NET_MAX_CONTEXT) {
		/* Not fast ack wait to send , stop timer */
		k_timer_stop(&fast_ack_timer);
	}
...
static void fast_ack_timer_callback(struct k_timer *timer)
{
	/* Do the work in  delay_ack_work */
	k_work_submit(&fast_ack_work);
}
#endif

#if defined(CONFIG_NET_TCP)
void net_context_set_ctrl_ack_flag(struct net_context *context, bool flag)
{
#if defined(CONFIG_NET_TCP_CTRL_ACK)
	if (context->tcp) {
		context->tcp->ctrl_ack = (flag)? 1 : 0;
	}
#endif
}

void net_context_ctrl_send_ack(struct net_context *context)
{
#if defined(CONFIG_NET_TCP_CTRL_ACK)
	struct net_conn *conn;
	int prio;
	...
	conn = (struct net_conn *)context->conn_handler;
	send_ack(context, &conn->remote_addr, false);
...
#if defined(USED_FAST_ACK)
	k_work_init(&fast_ack_work, tcp_fast_ack_work);
#endif
}
