...
#define TCP_SEND_CHECK_TIME		20		/* 20ms */
#endif
		       ...
		       void *user_data)
{
	...
	if (ctx->app_type == NET_APP_CLIENT) {
		...
		if (ctx->cb.recv) {
			...
			/* Not ower receive the pkt, need unref pkt */
			net_pkt_unref(pkt);
		}
	}
#endif
...
int net_app_release(struct net_app_ctx *ctx)
{
...
#if defined(CONFIG_NET_IPV6)
	if (ctx->ipv6.ctx) {
		if (!ctx->context_tcp_release) {
			net_context_put(ctx->ipv6.ctx);
		}
		ctx->ipv6.ctx = NULL;
		memset(&ctx->ipv6, 0, sizeof(struct net_app_endpoint));
	}
#endif /* CONFIG_NET_IPV6 */
	...
	if (ctx->ipv4.ctx) {
		...
		ctx->ipv4.ctx = NULL;
		memset(&ctx->ipv4, 0, sizeof(struct net_app_endpoint));
	}
#endif /* CONFIG_NET_IPV4 */

	ctx->is_init = false;
	ctx->context_tcp_release = false;
		     ...
		     void *user_data_send)
{
	...
	if (!ctx->is_init || ctx->context_tcp_release) {
		return -ENOENT;
	}
...
#if defined(CONFIG_NET_TCP)
		if ((ret == -EAGAIN) && (ctx->proto == IPPROTO_TCP)) {
			if (!ctx->is_init || ctx->context_tcp_release) {
				return -ENOENT;
			}

			if (keep_timeout == K_FOREVER) {
				k_sleep(TCP_SEND_CHECK_TIME);
			} else if (keep_timeout == 0) {
				SYS_LOG_WRN("TCP send timeout: %d", timeout);
				return ret;
			} else {
				keep_timeout = (keep_timeout > TCP_SEND_CHECK_TIME)? (keep_timeout - TCP_SEND_CHECK_TIME) : 0;
				k_sleep(TCP_SEND_CHECK_TIME);
			}
