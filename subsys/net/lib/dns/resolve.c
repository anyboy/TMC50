...
#define DNS_MAX_NAME_LEN	CONFIG_NET_NBUF_INNER_SIZE
#else
		    ...
		    void *user_data)
{
	...
	memset(&info, 0, sizeof(struct dns_addrinfo));
	ARG_UNUSED(net_ctx);
...
#ifdef DNS_USE_PKT_DATA_BUF
	dns_data = net_pkt_get_reserve_tx_data(0, ctx->buf_timeout);
#else
...
#ifdef DNS_USE_PKT_DATA_BUF
#if defined(CONFIG_NET_NBUF_INNER)
	dns_cname = net_pkt_get_reserve_inner_data(0, ctx->buf_timeout);
#else
	dns_cname = net_pkt_get_reserve_tx_data(0, ctx->buf_timeout);
#endif
#else
	...
	/* If not run to here, need to unref pkt */
		     ...
		     struct net_buf *dns_qname)
{
	...
	if (ctx->queries[query_idx].cb == NULL) {
		ret = 0;
	} else {
		if (k_delayed_work_remaining_get(&ctx->queries[query_idx].timer) > 0) {
			k_delayed_work_cancel(&ctx->queries[query_idx].timer);
...
#ifdef DNS_USE_PKT_DATA_BUF
#if defined(CONFIG_NET_NBUF_INNER)
	dns_qname = net_pkt_get_reserve_inner_data(0, ctx->buf_timeout);
#else
	dns_qname = net_pkt_get_reserve_tx_data(0, ctx->buf_timeout);
#endif
#else
...
void dns_update_server_addr(struct in_addr *dns_server)
{
	static struct in_addr dns0_addr = { { { 0, 0, 0, 0 } } };
	static struct in_addr dns1_addr = { { { 0, 0, 0, 0 } } };

	if (dns0_addr.s_addr == 0) {
		dns0_addr.s_addr = net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr;
		if (CONFIG_DNS_RESOLVER_MAX_SERVERS > 1) {
			dns1_addr.s_addr = net_sin(&dns_default_ctx.servers[1].dns_server)->sin_addr.s_addr;
		}
	}

	if (CONFIG_DNS_RESOLVER_MAX_SERVERS > 1) {
		if (dns_server->s_addr == 0) {
			net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr = dns0_addr.s_addr;
			net_sin(&dns_default_ctx.servers[1].dns_server)->sin_addr.s_addr = dns1_addr.s_addr;
		} else if (dns0_addr.s_addr == dns_server->s_addr) {
			net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr = dns_server->s_addr;
			net_sin(&dns_default_ctx.servers[1].dns_server)->sin_addr.s_addr = dns1_addr.s_addr;
		} else {
			net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr = dns_server->s_addr;
			net_sin(&dns_default_ctx.servers[1].dns_server)->sin_addr.s_addr = dns0_addr.s_addr;
		}
	} else {
		if (dns_server->s_addr == 0) {
			net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr = dns0_addr.s_addr;
...
void dns_exchange_server_addr(void)
{
	struct in_addr dns;

	if (CONFIG_DNS_RESOLVER_MAX_SERVERS >= 2) {
		dns.s_addr = net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr;
		net_sin(&dns_default_ctx.servers[0].dns_server)->sin_addr.s_addr = \
			net_sin(&dns_default_ctx.servers[1].dns_server)->sin_addr.s_addr;
		net_sin(&dns_default_ctx.servers[1].dns_server)->sin_addr.s_addr = dns.s_addr;
	}
}