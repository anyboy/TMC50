...
int dns_unpack_answer(struct dns_msg_t *dns_msg, int dname_ptr, u32_t *ttl)
{
	...
	if (answer[0] < DNS_LABEL_MAX_SIZE) {
		...
		if (buf_size < (dname_size + DNS_ANSWER_MIN_SIZE - 2)) {
			return -ENOMEM;
		}
	} else {
		...
		dname_size = DNS_COMMON_UINT_SIZE;
	}
	 ...
	 */
	if (dns_answer_class(dname_size, answer) != DNS_CLASS_IN) {
		return -EINVAL;
	}

	/* TTL value */
	*ttl = dns_answer_ttl(dname_size, answer);
	pos = dns_msg->answer_offset + DNS_ANSWER_MIN_SIZE + dname_size - 2;
	len = dns_answer_rdlength(dname_size, answer);

	switch (dns_answer_type(dname_size, answer)) {
	case DNS_RR_TYPE_A:
	case DNS_RR_TYPE_AAAA:
		set_dns_msg_response(dns_msg, DNS_RESPONSE_IP, pos, len);
		dns_msg->answer_offset += (dname_size - DNS_COMMON_UINT_SIZE);
		return 0;
