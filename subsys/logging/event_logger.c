...
{
	ring_buf_init(&logger->ring_buf, buffer_size, logger_buffer);
			     ...
			     void (*sem_give_fn)(struct k_sem *))
{
	...
	ret = ring_buf_item_put(&logger->ring_buf, event_id,
			    ...
			    u32_t *buffer, u8_t *buffer_size)
{
	...
	ret = ring_buf_item_get(&logger->ring_buf, event_id, dropped_event_count,
			       buffer, buffer_size);
	if (likely(!ret)) {
		return *buffer_size;
	}
...
int sys_event_logger_get_wait_timeout(struct event_logger *logger,
				      ...
				      u32_t timeout)
{
	if (k_sem_take(&(logger->sync_sema), __ticks_to_ms(timeout)) == 0) {
		return event_logger_get(logger, event_id, dropped_event_count,
					buffer, buffer_size);
	}
	return 0;
