...
NET_BUF_POOL_DEFINE(data_bredr_pool, 1, DATA_BREDR_MTU, BT_BUF_USER_DATA_MIN, NULL);
...
static struct bt_sdp_attribute spp_attrs[] = {
	...
	BT_SDP_LIST(
		...
		BT_SDP_DATA_ELEM_LIST_CONST(
		{
			...
			BT_SDP_ARRAY_16_CONST(BT_SDP_SERIAL_PORT_SVCLASS)
		},
		)
	),
	BT_SDP_LIST(
		...
		},
		{
			...
			},
			{
				...
				BT_SDP_ARRAY_8_CONST(BT_RFCOMM_CHAN_SPP)
			},
			)
		},
		)
...
struct store_keys_link_key br_link_key[MAX_BR_LINK_KEY];
...
static struct store_keys_link_key *bt_keys_find_link_key(const bt_addr_t *addr)
{
	...
	for (i = 0; i < MAX_BR_LINK_KEY; i++) {
		if (!bt_addr_cmp(&br_link_key[i].addr, addr)) {
			    ...
			    size_t length)
{
	...
	} else if (key == BT_STORAGE_REQ_LINK_KEY && length == LINK_KEY_LEN) {
			     ...
			     const void *data, size_t length)
{
	...
	if (key == BT_STORAGE_ID_ADDR && length == sizeof(id_addr)) {
		...
		return sizeof(id_addr);
	} else if (key == BT_STORAGE_NOTIFY_LINK_KEY && length == LINK_KEY_LEN) {
		link_key = bt_keys_find_link_key(&addr->a);
		if (!link_key) {
		...
		if (link_key) {
			bt_addr_copy(&link_key->addr, &addr->a);
...
#ifdef CONFIG_SECTION_OVERLAY
	overlay_section_init(OVERLAY_ID_LIBBTDRV);
#endif
...
static int cmd_advertise(int argc, char *argv[])
{
	...
	len = bt_read_ble_name(ble_name, 32);
...
static const struct bt_conn_auth_cb auth_cb_input = {
...
static const struct bt_conn_auth_cb auth_cb_all = {
	.passkey_display = auth_passkey_display,
	.passkey_entry = auth_passkey_entry,
	.passkey_confirm = auth_passkey_confirm,
#if defined(CONFIG_BT_BREDR)
...
static void br_discovery_result(struct bt_br_discovery_result *result)
{
	...
	if (result == NULL) {
		printk("BR/EDR discovery finish\n");
		return;
	}

	bt_addr_to_str((const bt_addr_t *)&result->addr, br_addr, sizeof(br_addr));
	memset(name, 0, sizeof(name));
...
static int cmd_bredr_discovery(int argc, char *argv[])
{
	...
	if (!strcmp(action, "on")) {
		...
		if (bt_br_discovery_start(&param, br_discovery_result) < 0) {
...
static int rfcomm_bredr_accept(struct bt_conn *conn, struct bt_rfcomm_dlc **dlc, u8_t channel)
{
...
#define DEBUG_BLE_SHELL_TEST	1

#if DEBUG_BLE_SHELL_TEST
...
/* static struct bt_gatt_service gatt_svc = BT_GATT_SERVICE(shell_attrs); */
...
static int cmd_ble_indicate(int argc, char *argv[])
{
	u8_t data[20], i;
	static u8_t data_start = '0';
	...
	if (!g_indicating) {
		printk("Client not set indicate!\n");
		return -1;
	}

	g_ind_params.attr = gatt_indicate_att;
	g_ind_params.func = __indicate_cb;
	g_ind_params.len = WECHAT_MAX_PKT;

	memset(data, 0, 20);
	for (i = 0; i < WECHAT_MAX_PKT; i++) {
		data[i] = data_start;
		data_start++;
		if (data_start > '9') {
			data_start = '0';
	...
	g_ind_params.data = data;
	bt_gatt_indicate(default_conn, &g_ind_params);
...
static const struct shell_cmd bt_commands[] = {
	...
	{ "ble-indicate", cmd_ble_indicate, "Ble indicate"},
#endif
	{ NULL, NULL, NULL }
};
