...
#define ECC_STACK_SIZE		(1280 + sizeof(struct k_thread) + BT_STACK_DEBUG_EXTRA)
static u8_t *ecc_thread_stack;
#else
...
static int ecc_start_thread(void)
{
	...
	ecc_thread_stack = host_malloc(ECC_STACK_SIZE);
	if (!ecc_thread_stack) {
		return -ENOMEM;
	}
...
static void bt_hci_ecc_env_init(void)
{
