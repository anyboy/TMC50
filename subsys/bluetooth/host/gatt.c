...
#define GATT_OPEN_BASE_SERVICE		0		/* Not need open */
...
static void bt_gatt_env_init(void)
{
#if defined(CONFIG_BT_GATT_CLIENT)
	sys_slist_init(&subscriptions);
#endif /* CONFIG_BT_GATT_CLIENT */

	sys_slist_init(&db);

#if GATT_OPEN_BASE_SERVICE
	memset(&gatt_svc, 0, sizeof(gatt_svc));
	gatt_svc.attrs = gatt_attrs;
	gatt_svc.attr_count = ARRAY_SIZE(gatt_attrs);

	memset(&gap_svc, 0, sizeof(gap_svc));
	gap_svc.attrs = gap_attrs;
	gap_svc.attr_count = ARRAY_SIZE(gap_attrs);

	memset(&sc_ccc_cfg, 0, sizeof(sc_ccc_cfg));
	memset(&gatt_sc, 0, sizeof(gatt_sc));
#endif
}
		   ...
		   const void *data, u16_t len)
{
	...
	if (conn) {
		if (!bt_le_conn_ready_send_data(conn)) {
			return -EBUSY;
		}
...
static void gatt_find_group_rsp(struct bt_conn *conn, u8_t err,
			       const void *pdu, u16_t length,
			       void *user_data)
{
	...
	/* Parse group found */
	length -= 1;
	for (i = 0; length >= rsp->len; i++, length -= rsp->len) {
		struct bt_gatt_attr attr = {};
		struct bt_gatt_service_val value;

		start_handle = sys_le16_to_cpu(rsp->data[i].start_handle);
		end_handle = sys_le16_to_cpu(rsp->data[i].end_handle);
...
static int gatt_find_group(struct bt_conn *conn,
			 struct bt_gatt_discover_params *params)
{
	struct net_buf *buf;
	...
	return gatt_send(conn, buf, gatt_find_group_rsp, params, NULL);
}
		     ...
		     struct bt_gatt_discover_params *params)
{
	...
	case BT_GATT_DISCOVER_GROUP:
		return gatt_find_group(conn, params);
	default:
...
void remove_subscription(struct bt_conn *conn, void *params)
{
	struct bt_gatt_subscribe_params *tmp, *next;
	sys_snode_t *prev = NULL;
...
u8_t disable_wa_force_to_write_ccc = false;
static void add_subscriptions(struct bt_conn *conn)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&subscriptions, params, node) {
		...
		//	       NULL, params);
		if (!disable_wa_force_to_write_ccc) {
			gatt_write_ccc(conn, params->ccc_handle, params->value,
			       gatt_write_ccc_rsp, params);
		}
	}
