...
static struct bt_sdp bt_sdp_pool[CONFIG_BT_MAX_BR_CONN] __in_section_unique(bthost_bss);

/* Pool for outgoing SDP packets */
extern struct net_data_pool host_tx_pool;
BT_BUF_POOL_DEFINE(sdp_pool, CONFIG_BT_MAX_BR_CONN,
		    BT_L2CAP_BUF_SIZE(SDP_MTU), BT_BUF_USER_DATA_MIN, NULL, &host_tx_pool);
...
static struct bt_sdp_client bt_sdp_client_pool[CONFIG_BT_MAX_BR_CONN] __in_section_unique(bthost_bss);
...
static void bt_sdp_disconnected(struct bt_l2cap_chan *chan)
{
	...
	/*
	 * Reset sdp excluding L2CAP channel member. Let's the channel
	 * resets autonomous.
	 */
	memset(&sdp->partial_resp_queue, 0, sizeof(*sdp) - sizeof(sdp->chan));
}
			    ...
			    struct net_buf *buf, u8_t nest_level)
{
	...
	if ((elem->type & BT_SDP_TYPE_DESC_MASK) == BT_SDP_SEQ_UNSPEC ||
		...
		if (seq_size == 1) {
			...
			if (seq_size == 16) {
				sys_memcpy_swap(tmp_buf, elem->data, seq_size);
				net_buf_add_mem(buf, tmp_buf, seq_size);
			} else {
				/* TODO: Convert 32bit and 128bit values to big-endian*/
				BT_INFO("Need convert1 %d ??", seq_size);
				net_buf_add_mem(buf, elem->data, seq_size);
			}
		}
	} else {
		BT_INFO("Need convert2 %d ??", seq_size);
		net_buf_add_mem(buf, elem->data, seq_size);
	}
...
static int bt_sdp_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_sdp_pool); i++) {
		...
		sdp->chan.chan.ops = (struct bt_l2cap_chan_ops *)&ops;
...
static void bt_sdp_env_init(void)
{
	net_buf_pool_reinitializer(&sdp_pool, CONFIG_BT_MAX_BR_CONN);
	memset(bt_sdp_client_pool, 0, sizeof(bt_sdp_client_pool));

	db = NULL;
	num_services = 0;
	memset(bt_sdp_pool, 0, sizeof(bt_sdp_pool));
}
...
static int sdp_client_ssa_search(struct bt_sdp_client *session)
{
	...
	ret = bt_l2cap_chan_send(&session->chan.chan, buf);
	if (ret < 0) {
		net_buf_unref(buf);
	}
...
static void sdp_client_receive(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	case BT_SDP_SVC_SEARCH_ATTR_RSP:
		...
		if (frame_len == 2 && cstate->length == 0 &&
			...
			goto iterate;
		} else if (frame_len == 3 && cstate->length == 0 &&
			session->cstate.length == 0 && buf->data[0] == 0x36) {
			/* Data Element size uint16 with data value MISSING */
			BT_DBG("record for UUID 0x%s not found",
				bt_uuid_str(session->param->uuid));
			/* Call user UUID handler */
			sdp_client_notify_result(session, UUID_NOT_RESOLVED);
			     ...
			     u16_t proto_profile)
{
	...
	while (p - attr->val < attr->len) {
		 ...
		 */
		if (((uuid_type == BT_SDP_UUID16) && (proto_profile == BT_UUID_16(&pd->uuid)->val)) ||
		    ((uuid_type == BT_SDP_UUID32) && (proto_profile == BT_UUID_32(&pd->uuid)->val))) {
			pd->params = p;
			pd->params_len = left;
