...
static sys_slist_t le_channels __in_section_unique(bthost_bss);
...
static struct bt_l2cap bt_l2cap_pool[CONFIG_BT_MAX_CONN] __in_section_unique(bthost_bss);
...
struct net_buf *bt_l2cap_create_pdu_len(struct net_buf_pool *pool, size_t reserve, u16_t len)
{
	u16_t malloc_len = len + sizeof(struct bt_l2cap_hdr) + reserve;

	return bt_conn_create_pdu_len(pool, sizeof(struct bt_l2cap_hdr) + reserve, malloc_len);
}
...
static int l2cap_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_l2cap_pool); i++) {
		...
		l2cap->chan.chan.ops = (struct bt_l2cap_chan_ops *)&ops;
...
static void bt_l2cap_env_init(void)
{
	memset(bt_l2cap_pool, 0, sizeof(bt_l2cap_pool));

	sys_slist_init(&le_channels);
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
...
void bt_l2cap_init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_BREDR)) {
		...
		extern void bt_rfcomm_init(void);
		bt_rfcomm_init();
#endif
#ifdef CONFIG_BT_AVDTP
		extern int bt_avdtp_init(void);
		bt_avdtp_init();
#endif
#ifdef CONFIG_BT_A2DP
		extern int bt_a2dp_init(void);
		bt_a2dp_init();
#endif
#ifdef CONFIG_BT_AVRCP
		extern int bt_avrcp_init(void);
		extern int bt_avrcp_cttg_init(void);
		bt_avrcp_init();
		bt_avrcp_cttg_init();
#endif
#ifdef CONFIG_BT_HFP_HF
		extern void bt_hfp_hf_init(void);
		bt_hfp_hf_init();
#endif
#ifdef CONFIG_BT_HFP_AG
		extern void bt_hfp_ag_init(void);
		bt_hfp_ag_init();
#endif

#ifdef CONFIG_BT_HID
		extern int bt_hiddev_init(void);
		bt_hiddev_init();
#endif
	}
}
