...
static struct bt_smp_br bt_smp_br_pool[CONFIG_BT_MAX_BR_CONN] __in_section_unique(bthost_bss);
#endif /* CONFIG_BT_BREDR */

static struct bt_smp bt_smp_pool[CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN] __in_section_unique(bthost_bss);
static bool sc_supported __in_section_unique(bthost_bss);
static bool sc_local_pkey_valid __in_section_unique(bthost_bss);
static u8_t sc_public_key[64] __in_section_unique(bthost_bss);
...
/* This Derivation of BR/EDR link key from LE LTK
 * so bt_keys_get_link_key from keys_br,
 * but not release when ble disconnected, lead to link_key leakage.
 * Not used Derivation function now, just use one static link_key for function running.
 */
static struct bt_keys_link_key ble_link_key;
...
static int bt_smp_br_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_smp_br_pool); i++) {
		...
		smp->chan.chan.ops = (struct bt_l2cap_chan_ops *)&ops;
...
static void bt_smp_env_init(void)
{
#if defined(CONFIG_BT_BREDR)
	memset(bt_smp_br_pool, 0, sizeof(bt_smp_br_pool));
#endif /* CONFIG_BT_BREDR */

	memset(bt_smp_pool, 0, sizeof(bt_smp_pool));
	sc_supported = false;
	sc_local_pkey_valid = false;
	memset(sc_public_key, 0, sizeof(sc_public_key));
}
