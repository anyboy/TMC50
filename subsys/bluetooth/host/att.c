...
BT_BUF_POOL_DEFINE(prep_pool, CONFIG_BT_ATT_PREPARE_COUNT, BT_ATT_MTU,
		    sizeof(struct bt_attr_data), NULL, &host_tx_pool);
...
static struct bt_att bt_req_pool[CONFIG_BT_MAX_CONN - CONFIG_BT_MAX_BR_CONN] __in_section_unique(bthost_bss);
...
static int bt_att_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_req_pool); i++) {
		...
		att->chan.chan.ops = (struct bt_l2cap_chan_ops *)&ops;
...
static void bt_att_env_init(void)
{
	net_buf_pool_reinitializer(&prep_pool, CONFIG_BT_ATT_PREPARE_COUNT);
	memset(bt_req_pool, 0, sizeof(bt_req_pool));
}
