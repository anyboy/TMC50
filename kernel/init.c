...
__in_section_unique(SRAM_NO_INIT) struct _k_thread_stack_element __aligned(STACK_ALIGN) _interrupt_stack[CONFIG_ISR_STACK_SIZE];
#else
__in_section_unique(interrupt.noinit.stack) struct _k_thread_stack_element __aligned(STACK_ALIGN) _interrupt_stack[CONFIG_ISR_STACK_SIZE];
#endif
...
#ifdef CONFIG_RAMFUNCTION_MEMORY
	memcpy(&__ramfunc_ram_start, &__ramfunc_rom_start,
...
static void prepare_multithreading(struct k_thread *dummy_thread)
{
	...
	/* run dummy_thread until switch to main thread */
	dummy_thread->base.prio = K_HIGHEST_THREAD_PRIO;
#endif
