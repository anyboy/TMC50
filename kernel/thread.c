...
int saved_always_on = k_enable_sys_clock_always_on();
	...
	for (;;) {
		...
		/* make up for cycles when tick timer irq is blocked
		 * e.g. k_busy_wait() is called in isr or irq is locked
		 */
		if (current_cycles < start_cycles)
			current_cycles  += CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC / CONFIG_SYS_CLOCK_TICKS_PER_SEC;
#endif
